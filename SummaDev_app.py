...
#NeuSummaDev
def NeuSummaDev(text):
    #load necessary objects to spit the text in sentences and tokenize on a word level
    nlp = spacy.load("en_core_web_lg")
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True)
    LSTM_model = keras.models.load_model("/Users/gregoireroquette/Documents/Python/Dissertation/models/LSTM_25_epochs")
    ...
    probabilities = LSTM_model.predict(X).tolist()
    predictions = [probabilities.index(x) for x in sorted(probabilities, reverse=True)[:2]]
    predictions.sort()
    summary = []
...
def BertSummaDev(text):
    ...
    nlp = spacy.load("en_core_web_lg")
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True)
    BERT_model = BertForSequenceClassification.from_pretrained("/Users/gregoireroquette/Documents/Python/Dissertation/models/BERT_2_epochs_jupiter")
...
def main():
    ...
    #Implementing a sidebar to chose between the two models
    activities = ['NeuSummaDev summariser', 'BertSummaDev summariser']
    choice = st.sidebar.selectbox('Chose your summariser', activities)

    if st.button('Summarise'):
        if choice == 'NeuSummaDev summariser':
            result_summary = NeuSummaDev(text)

        elif choice == 'BertSummaDev summariser':
